function scmaps_pre01_iso_dtitk(scmapsFolder,res,dims)
...
for i=1:length(brains)
    commandR=['SVResample -in ' scmapsFolder,filesep,brains(i).name ' -out ' [pathout,filesep,brains(i).name] ' -size '  num2str(dims(1)) ' ' num2str(dims(2)) ' ' num2str(dims(3))    ' -vsize ' num2str(res) ' ' num2str(res) ' ' num2str(res) ];
