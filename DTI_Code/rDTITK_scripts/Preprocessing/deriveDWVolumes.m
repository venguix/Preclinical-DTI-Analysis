...
if dw_bool     %% Derive DW images from raw data (LA)
    ...
    %%
    tmp = zeros(ud.fn,ud.fn1,size(fid2,3),size(fid2,4));
    ...
    for sliceno = 1:ud.ns
        ...
        tmp(:,:,sliceno,:)= abs(dtiImages);
        for dirno=1:size(tmp,4)
            img(:,:,sliceno,dirno)=tmp(:,:,sliceno,dirno)';%ici la transposee est pour etre en accord avec les cartes scalaires derivees dans deriveTensorandScalarMaps
        end
    end
    
    
    dirOut = strrep(pathtobrains,'Brains','DWVolumes');
    dirOut = [dirOut,filesep,'no_corr'];
    
    %ATTENTION CHECKS THE WHOLE SEARCH PATH, WE ADD './'
    if ~exist(['./' dirOut],'dir')
        mkdir(dirOut);
    end
   
    %% Sauvegarde
    % Pour output un volume 4D des DWI
    ...
    fileout=[dirOut filesep fidname '_DWIs.nii'];    
